...
describe("Recipe", () => {
  ...
  it("Should have a method that creates a filtered list of recipes based on a tag", () => {
    ...
    expect(allRecipes.filterByTag("snack")).to.deep.equal([
      recipeRepositorySampleData[0],
    ]);
    expect(allRecipes.filterByTag("lunch")).to.deep.equal(
      recipeRepositorySampleData
    );
  });
  ...
  it("Should have a method that creates a filtered list of recipes based on its name", () => {
    ...
    expect(
      allRecipes.filterByName("Maple Dijon Apple Cider Grilled Pork Chops")
    ).to.deep.equal([recipeRepositorySampleData[1]]);
  });
});

...
function calculateRecipeCost(recipe, ingredientsData) {
  const totalCost = (!recipe || !ingredientsData) ? 0 :
    recipe.ingredients.reduce((acc, ingredient) => {
      const ingredientCost = getIngredientProperty(ingredient.id, ingredientsData, 'estimatedCostInCents');
      return acc + (ingredientCost ? ingredientCost * ingredient.quantity.amount : 0);
    }, 0);
